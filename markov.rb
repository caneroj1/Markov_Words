  ...
  class Generator
    ...
    def populate_table
      ...
      while (corpus_index < corpus.length - @look_ahead)
        first_word = @corpus[corpus_index...corpus_index + @look_ahead]
        follower = @corpus[corpus_index + @look_ahead...corpus_index + 2 * @look_ahead]
    ...
    def word_from_weighted_hash(hash)
      result = ""
      sum = hash.each_value.inject { |value, sum| sum += value }
